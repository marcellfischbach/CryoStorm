...
csImage::csImage(uint16_t width, uint16_t height, ePixelFormat format)
...
void csImage::GenerateLayer(uint16_t layer)
{
...
void csImage::GenerateMipMaps(eMipMapProcedure procedure)
{
  if (!m_mipmapCreated)
  {
...
void csImage::GenerateMipMapLayers()
{
...
void csImage::Copy(uint16_t layer, const uint8_t *buffer)
{
  if (layer >= m_numberOfLayers)
  {
    return;
...
uint16_t csImage::GetNumberOfLayers() const
{
  return m_numberOfLayers;
}

uint16_t csImage::GetWidth(uint16_t layer) const
{
  if (layer >= m_numberOfLayers)
  {
    return 0;
...
uint16_t csImage::GetHeight(uint16_t layer) const
{
  if (layer >= m_numberOfLayers)
  {
    return 0;
...
const uint8_t *csImage::GetData(uint16_t layer) const
{
  if (layer >= m_numberOfLayers)
  {
    return nullptr;
...
ePixelFormat csImage::GetPixelFormat() const
{
  return m_pixelFormat;
}

void csImage::GenerateMipMapsLinear4x4()
{
...
void csImage::GenerateMipMapsNormal()
{
  for (uint16_t l = 1; l < m_numberOfLayers; ++l)
  {
    Layer &src = m_layers[l - 1];
...
void csImage::ColorCorrectionClamp3()
{
...
void csImage::ColorCorrectionClamp4()
{
...
void csImage::ColorCorrectionNormalize()
{
  uint32_t bpp   = byte_per_pixel(m_pixelFormat);
  if (bpp < 3)
  {
