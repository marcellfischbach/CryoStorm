...
GL4ForwardDirectionalLightRenderer::GL4ForwardDirectionalLightRenderer()
    : m_device(nullptr)
    , m_directionalLightShadowBuffer(nullptr)
...
{
  return GetDirectionalLightShadowBuffer()->GetColorTexture(0);
}

iTexture2DArray *GL4ForwardDirectionalLightRenderer::GetDepthTexture()
{
  return GetDirectionalLightShadowBuffer()->GetDepthTexture();
}
                                                          ...
                                                          const Projector &projector)
{
  ...
  for (auto directionalLight: m_shadowDirectionalLights)
  {
    ...
    RenderShadow(directionalLight, camera, projector);
  }
}
                                                         ...
                                                         const Projector &projector)
{
  ...
  GL4RenderTarget2D* target = GetDirectionalLightShadowMap();
  ...
  unit = m_device->BindTexture(m_depthBuffer);
...
GL4RenderTarget2D *GL4ForwardDirectionalLightRenderer::GetDirectionalLightShadowMap()
{
  if (m_directionalLightShadowMap)
  {
    if (m_directionalLightShadowMapWidth == m_directionalLightShadowMap->GetWidth()
    && m_directionalLightShadowMapHeight == m_directionalLightShadowMap->GetHeight())
    {
      return m_directionalLightShadowMap;
    }
    m_directionalLightShadowMap->Release();
    m_directionalLightShadowMap = nullptr;
  }

  iRenderTarget2D::Descriptor desc {};
  desc.Width = m_directionalLightShadowMapWidth;
  desc.Height = m_directionalLightShadowMapHeight;
  m_directionalLightShadowMap = QueryClass<GL4RenderTarget2D>(m_device->CreateRenderTarget(desc));

  iTexture2D::Descriptor colorDesc {};
  colorDesc.Width = m_directionalLightShadowMapWidth;
  colorDesc.Height = m_directionalLightShadowMapHeight;
  colorDesc.Format = ePF_RGB;
  colorDesc.MipMaps = false;
  iTexture2D *colorTexture = m_device->CreateTexture(colorDesc);
  m_directionalLightShadowMap->AddColorTexture(colorTexture);

  m_directionalLightShadowMap->SetDepthBuffer(ePF_Depth);
