...
#include <SpiceCore/math/vector4f.hh>
...
struct SPICE_CORE_API Matrix4f
{
public:
  float m00;
  float m01;
...
public:
  SPICE_FORCEINLINE Matrix4f(const float* f)
    : m00(f[0])
    , m01(f[1])
    , m02(f[2])
    , m03(f[3])
  ...
  SPICE_FORCEINLINE Matrix4f(float m00 = 1.0f, float m01 = 0.0f, float m02 = 0.0f, float m03 = 0.0f, float m10 = 0.0f, float m11 = 1.0f, float m12 = 0.0f, float m13 = 0.0f, float m20 = 0.0f, float m21 = 0.0f, float m22 = 1.0f, float m23 = 0.0f, float m30 = 0.0f, float m31 = 0.0f, float m32 = 0.0f, float m33 = 1.0f)
    : m00(m00)
    , m01(m01)
    , m02(m02)
    , m03(m03)
  ...
  SPICE_FORCEINLINE void SetIdentity()
  {
    m01 = m02 = m03 = m10 = m12 = m13 = m20 = m21 = m23 = m30 = m31 = m32 = 0.0f;
    m00 = m11 = m22 = m33 = 1.0f;
  }

  SPICE_FORCEINLINE void Set(float m00 = 1.0f, float m01 = 0.0f, float m02 = 0.0f, float m03 = 0.0f, float m10 = 0.0f, float m11 = 1.0f, float m12 = 0.0f, float m13 = 0.0f, float m20 = 0.0f, float m21 = 0.0f, float m22 = 1.0f, float m23 = 0.0f, float m30 = 0.0f, float m31 = 0.0f, float m32 = 0.0f, float m33 = 1.0f)
  {
    this->m00 = m00;
    this->m01 = m01;
    this->m02 = m02;
  ...
  SPICE_FORCEINLINE void Set(const Matrix4f& matrix)
  {
    m00 = matrix.m00;
    m01 = matrix.m01;
    m02 = matrix.m02;
  ...
  SPICE_FORCEINLINE Vector3f& GetXAxis(Vector3f& res) const
  {
    res.x = m00;
    res.y = m01;
    res.z = m02;
  ...
  SPICE_FORCEINLINE Vector3f& GetYAxis(Vector3f& res) const
  {
    res.x = m10;
    res.y = m11;
    res.z = m12;
  ...
  SPICE_FORCEINLINE Vector3f& GetZAxis(Vector3f& res) const
  {
    res.x = m20;
    res.y = m21;
    res.z = m22;
  ...
  SPICE_FORCEINLINE Vector3f& GetAxis(UInt8 axis, Vector3f& res) const
  {
    switch (axis)
    {
    case 0:
  ...
  SPICE_FORCEINLINE void ClearRotation()
  {
    m00 = m11 = m22 = 1.0f;
    m01 = m02 = m10 = m12 = m20 = m21 = 0.0f;
  }

  SPICE_FORCEINLINE void SetTranslation(float x, float y, float z, float w = 1.0f)
  {
    m30 = x;
    m31 = y;
    m32 = z;
  ...
  SPICE_FORCEINLINE void SetTranslation(const Vector3f& tr, float w = 1.0f)
  {
    m30 = tr.x;
    m31 = tr.y;
    m32 = tr.z;
  ...
  SPICE_FORCEINLINE Vector3f& GetTranslation(Vector3f& res) const
  {
    res.x = m30;
    res.y = m31;
    res.z = m32;
  ...
  SPICE_FORCEINLINE Matrix4f& SetRotationX(float angle)
  {
  ...
  SPICE_FORCEINLINE Matrix4f& SetRotationY(float angle)
  {
    float c = (float)cos(angle);
    float s = (float)sin(angle);
    m00 = c;
  ...
  SPICE_FORCEINLINE Matrix4f& SetRotationZ(float angle)
  {
    float c = (float)cos(angle);
    float s = (float)sin(angle);
    m00 = c;
  ...
  SPICE_FORCEINLINE Matrix4f& SetRotation(const Vector3f& axis, float angle)
  {
    float c = cosf(angle);
    float s = sinf(angle);
  ...
  SPICE_FORCEINLINE Matrix4f& Inverted(Matrix4f& res) const
  {
  ...
  SPICE_FORCEINLINE Matrix4f & Invert()
  {
    float v0 = m20 * m31 - m21 * m30;
    float v1 = m20 * m32 - m22 * m30;
    float v2 = m20 * m33 - m23 * m30;
  ...
  SPICE_FORCEINLINE Matrix4f Transposed() const
  {
    return Matrix4f(
      m00, m10, m20, m30,
      m01, m11, m21, m31,
...
#define SWAP(a, b, s) (s) = (a); (a)=(b); (b)=(s)
  SPICE_FORCEINLINE Matrix4f & Transpose()
  {
    float s;
    SWAP(m01, m10, s);
    SWAP(m02, m20, s);
...
#undef SWAP
  ...
  SPICE_FORCEINLINE Matrix4f& SetLookAt(const Vector3f & eye, const Vector3f & spot, const Vector3f & up)
  {
  ...
  SPICE_FORCEINLINE Matrix4f& SetLookAtInv(const Vector3f & eye, const Vector3f & spot, const Vector3f & up)
  {
    Vector3f xAxis, yAxis, zAxis;
    Vector3f::Sub(eye, spot, zAxis).Normalize();
    Vector3f::Cross(up, zAxis, xAxis).Normalize();
  ...
  SPICE_FORCEINLINE static Matrix4f& Mult(const Matrix4f & m0, const Matrix4f & m1, Matrix4f & r)
  {
  ...
  SPICE_FORCEINLINE void Debug(const char* message = 0) const
  {
    if (message)
    {
      printf("csMatrix: %s\n", message);
  ...
  SPICE_FORCEINLINE bool IsIdentity() const
  {
    return SPICE_FLOAT_IS_ONE(m00) &&
      SPICE_FLOAT_IS_ONE(m11) &&
      SPICE_FLOAT_IS_ONE(m22) &&
      SPICE_FLOAT_IS_ONE(m33) &&
      SPICE_FLOAT_IS_ZERO(m01) &&
      SPICE_FLOAT_IS_ZERO(m02) &&
      SPICE_FLOAT_IS_ZERO(m03) &&
      SPICE_FLOAT_IS_ZERO(m10) &&
      SPICE_FLOAT_IS_ZERO(m12) &&
      SPICE_FLOAT_IS_ZERO(m13) &&
      SPICE_FLOAT_IS_ZERO(m20) &&
      SPICE_FLOAT_IS_ZERO(m21) &&
      SPICE_FLOAT_IS_ZERO(m23) &&
      SPICE_FLOAT_IS_ZERO(m30) &&
      SPICE_FLOAT_IS_ZERO(m31) &&
      SPICE_FLOAT_IS_ZERO(m32);
  }
