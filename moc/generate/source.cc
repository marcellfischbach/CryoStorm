...
std::string ClassGenerator::GenerateFunctionClass(ClassNode* classNode, std::list<NamespaceNode*>& nss, FunctionNode* function, const std::string&, CSMetaNode*)
{
  ...
  source += "\nclass " + fnctClassName + " : public cs::Function\n";
  source += "{\n";
  source += "public:\n";

  // create the constructor
  source += "  " + fnctClassName + "()\n";
  source += std::string("    : cs::Function (") +
...
std::string ClassGenerator::GenerateFunctionVoidInvokeMethod(ClassNode *classNode, FunctionNode *function, bool constMethod)
{
  ...
  {
    fkt += "    throw cs::BadMethodInvokation(\"[" + function->GetName() + "] No const-call\");\n";
  }
  else
  {
    fkt += "    " + sConst + classNode->GetName() + " *d = cs::QueryClass<" + sConst + classNode->GetName() + ">(obj);\n";
    fkt += "    if (!d)\n";
    fkt += "    {\n";
    fkt += "      throw cs::BadMethodInvokation(\"[" + function->GetName() + "] Object is not instance of " + classNode->GetName() + "\");\n";
    fkt += "    }\n\n";
...
std::string ClassGenerator::GenerateFunctionValueInvokeMethod(ClassNode *classNode, FunctionNode *function, bool constMethod)
{
  ...
  {
    fkt += "    throw cs::BadMethodInvokation(\"[" + function->GetName() + "] No value-call\");\n";
  }
...
std::string ClassGenerator::GenerateFunctionReferenceInvokeMethod(ClassNode *classNode, FunctionNode *function, bool constReturn, bool constMethod)
{
  ...
  fkt += "\n";
  fkt += "  virtual " + rConst + "void *Invoke" + mConst + "ReferenceImpl (" + sConst + "cs::iObject* obj, ...) const\n";
  fkt += "  {\n";
  if (!function->GetReturnValue().IsReference() )
  {
    fkt += "    throw cs::BadMethodInvokation(\"[" + function->GetName() + "] No reference-call\");\n";
  }
  else if (!constReturn && function->GetReturnValue().IsConst())
  {
    fkt += "    throw cs::BadMethodInvokation(\"[" + function->GetName() + "] Return value is non-const\");\n";
  }
  else if (constMethod && !function->IsConst())
  {
    fkt += "    throw cs::BadMethodInvokation(\"[" + function->GetName() + "] No const-call\");\n";
...
std::string ClassGenerator::GenerateFunctionPointerInvokeMethod(ClassNode *classNode, FunctionNode *function, bool constReturn, bool constMethod)
{
  ...
  {
    fkt += "    throw cs::BadMethodInvokation(\"[" + function->GetName() + "] No pointer-call\");\n";
  }
  else if (!constReturn && function->GetReturnValue().IsConst())
  {
    fkt += "    throw cs::BadMethodInvokation(\"[" + function->GetName() + "] Return value is non-const\");\n";
  ...
  else
  {
    ...
    fkt += "    if (!d)\n";
    fkt += "    {\n";
    fkt += "      throw cs::BadMethodInvokation(\"Object is not instance of " + classNode->GetName() + "\");\n";
    fkt += "    }\n\n";
...
std::string ClassGenerator::GenerateAttribute(ClassNode* classNode, std::list<NamespaceNode*>& nss, MemberNode* member, const std::string& visibility, CSMetaNode* meta)
{
  ...
  prop += "\nclass " + propClassName + " : public cs::Property\n";
  prop += "{\n";
  prop += "public:\n";
  prop += "  " + propClassName + "()\n";
  prop += "    : cs::Property(" + GenerateCSValueDeclaration(containerType, false) + ", \"" + propName + "\", " + GenerateCSValueDeclaration(type, true) + ")\n";
  prop += "  {\n";
  prop += "  }\n\n";
  ...
  {
    prop += "    " + classNode->GetName() + " *d = cs::QueryClass<" + classNode->GetName() + ">(object);\n";
    prop += "    if (!d)\n";
    prop += "    {\n";
    prop += "      throw cs::BadMethodInvokation(\"Object is not instance of " + classNode->GetName() + "\");\n";
    prop += "    }\n\n";
  ...
  {
    prop += "    throw cs::BadMethodInvokation(\"Property [" + classNode->GetName() + "::" + member->GetName() + "] is not public\");\n";
  }
  prop += "  }\n\n";
  ...
  {
    prop += "    const " + classNode->GetName() + " *d = cs::QueryClass<const " + classNode->GetName() + ">(object);\n";
    prop += "    if (!d)\n";
    prop += "    {\n";
    prop += "      throw cs::BadMethodInvokation(\"Object is not instance of " + classNode->GetName() + "\");\n";
    prop += "    }\n\n";
...
std::string ClassGenerator::GenerateCSValueDeclaration(const TypeDef& typeDef, bool withSubTypes)
{
  ...
  return std::string("cs::ValueDeclaration(") +
...
{
  return std::string("AddAttribute(cs::FunctionAttribute (") +
...
std::string ClassGenerator::GenerateClass(ClassNode * classNode, std::list<NamespaceNode*> & nss, CSMetaNode * meta)
{
  ...
  {
    cls += "  throw cs::InstanciationException(\"" + fns + className + "\");";
  }
  cls += "}\n\n";


  cls += "const cs::Class *" + fns + className + "::GetClass() const\n";
  cls += "{\n";
  cls += "  return " + fns + classClassName + "::Get();\n";
  cls += "}\n\n";
  cls += "const cs::Class *" + fns + className + "::GetStaticClass()\n";
  cls += "{\n";
  cls += "  return " + fns + classClassName + "::Get();\n";
  cls += "}\n\n";
...
std::string ClassGenerator::GenerateQueryClass(ClassNode * classNode, std::list<NamespaceNode*> & nss, CSMetaNode *, bool _const)
{
  ...
  query += sConst + "void *" + fns + className + "::QueryClass(const cs::Class* clazz) " + sConst + "\n";
  query += "{\n";
