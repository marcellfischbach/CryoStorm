...
#define CHECK_IDX(tokens, idx) if ((idx) >= tokens.size()) throw Spice::moc::ParseException(__FILE__, __LINE__, "Index out of boundx " + std::to_string(idx) + " >= " + std::to_string(tokens.size()))
...
ClassSuperDefinition Parser::ParseSuperDefinition(Tokenizer & tokenizer, size_t & idx, ASTNode * parent)
{
  ...
  for (; idx < tokens.size(); )
  {
    ...
    case eTT_Identifier:
      CHECK_IDX(tokens, idx + 1);
      if (token.Get() == std::string("SPICE_SUPER") && idx + 1 < tokens.size() && tokens[idx + 1].GetType() == eTT_ParenOpen)
      {
        csSuper = true;
        idx += 2;
      }
...
TypeDef Parser::GetType(Tokenizer & tokenizer, size_t & idx)
{
  ...
  while (true)
  {
    ...
    if (token.GetType() == eTT_Identifier)
    {
      if (token.Get() == std::string("SPICE_FORCEINLINE")
