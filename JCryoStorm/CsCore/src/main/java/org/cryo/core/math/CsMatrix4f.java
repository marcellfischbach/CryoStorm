...
public class CsMatrix4f
{
    ...
    public static final CsMatrix4f IDENTITY = new CsMatrix4f();
    ...
    public CsMatrix4f set(float m00, float m01, float m02, float m03,
                          float m10, float m11, float m12, float m13,
                          float m20, float m21, float m22, float m23,
                          float m30, float m31, float m32, float m33
                       ) {
    ...
    public CsMatrix4f set(CsMatrix4f m) {
        this.m00 = m.m00;
        this.m01 = m.m01;
        this.m02 = m.m02;
        this.m03 = m.m03;
    ...
    public void setXAxis(CsVector3f xAxis) {
        this.m00 = xAxis.x;
        this.m01 = xAxis.y;
        this.m02 = xAxis.z;
        this.m03 = 0.0f;
    ...
    public CsVector3f getXAxis(CsVector3f out) {
        return out.set(m00, m01, m02);
    }

    public void setYAxis(CsVector3f yAxis) {
        this.m10 = yAxis.x;
        this.m11 = yAxis.y;
        this.m12 = yAxis.z;
        this.m13 = 0.0f;
    ...
    public CsVector3f getYAxis(CsVector3f out) {
        return out.set(m10, m11, m12);
    }

    public void setZAxis(CsVector3f zAxis) {
        this.m20 = zAxis.x;
        this.m21 = zAxis.y;
        this.m22 = zAxis.z;
        this.m23 = 0.0f;
    ...
    public CsVector3f getZAxis(CsVector3f out) {
    ...
    public void setScale(CsVector3f s) {
        m00 = s.x;
        m11 = s.y;
        m22 = s.z;
    }

    public void scale(CsVector3f s) {
        m00 *= s.x;
        m11 *= s.y;
        m22 *= s.z;
    }

    public CsMatrix4f setRotation(CsVector3f axis, float angle) {
        float c = (float) Math.cos(angle);
        float s = (float) Math.sin(angle);
    ...
    public CsMatrix4f mul(CsMatrix4f m, CsMatrix4f out) {
    ...
    public CsVector3f mul(CsVector3f v, CsVector3f out) {
    ...
    public CsVector3f transform(CsVector3f v, CsVector3f out) {
    ...
    public CsMatrix4f setLookAt(CsVector3f eye, CsVector3f spot, CsVector3f up) {

        CsVector3f zAxis = new CsVector3f(spot).sub(eye).normalize();
        CsVector3f xAxis = new CsVector3f(up).cross(zAxis).normalize();
        CsVector3f yAxis = new CsVector3f(zAxis).cross(xAxis).normalize();
    ...
    public CsMatrix4f setLookAtInv(CsVector3f eye, CsVector3f spot, CsVector3f up) {
        CsVector3f zAxis = new CsVector3f(spot).sub(eye).normalize();
        CsVector3f xAxis = new CsVector3f(up).cross(zAxis).normalize();
        CsVector3f yAxis = new CsVector3f(zAxis).cross(xAxis);
    ...
    public CsMatrix4f Inverted() {
        ...
        return new CsMatrix4f(lm00, lm01, lm02, lm03,
                              lm10, lm11, lm12, lm13,
                              lm20, lm21, lm22, lm23,
                              lm30, lm31, lm32, lm33);
    }

    public CsMatrix4f invert() {
        float v0 = m20 * m31 - m21 * m30;
        float v1 = m20 * m32 - m22 * m30;
        float v2 = m20 * m33 - m23 * m30;
        float v3 = m21 * m32 - m22 * m31;
